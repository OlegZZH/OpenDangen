...
class Surface:
    def __init__(self):
        ...
        self.index = np.array([])
        self.size = 20
        self.t2 = np.linspace(0, 1, self.size)
    ...
    def plot_surf(self, points):
        p2 = 2
        p3 = 3
        kh2 = len(points.reshape(self.h, self.w, -1)[0][p2::p2])
        kh3 = len(points.reshape(self.h, self.w, -1)[0][p3::p3])
        kw2 = 1  # len(points.reshape(self.h, self.w, -1)[:, 0][p2::p2])
        kw3 = 1  # len(points.reshape(self.h, self.w, -1)[:, 0][p2::p2])

        pool_of_points = np.array(
            [np.array([i[j], i[j + 1], i[j + 2]]) for i in points.reshape(self.h, self.w, -1)[:3] for j in
             range(0, kh2 * p2, p2)]).reshape(-1, 3)
        row_Cur_2 = pool.map(self.biz2, pool_of_points.reshape((p2 + 1) * kh2, -1, 3))

        c = np.array(row_Cur_2).reshape((p2 + 1) * kw2, -1, 3)
        pool_of_points = np.array([[c[j][i], c[j + 1][i], c[j + 2][i]] for i in range(self.size * kh2) for j in
                                   range(0, kw2 * p2, p2)])
        Cur = np.array(pool.map(self.biz2, pool_of_points))

        pool_of_points = np.array(
            [np.array([i[j], i[j + 1], i[j + 2], i[j + 3]]) for i in points.reshape(self.h, self.w, -1)[2:] for j in
             range(0, kh3 * p3, p3)]).reshape(-1, 3)
        row_Cur_3 =np.array(pool.map(self.biz3, pool_of_points.reshape((p3 + 1) * kh3, -1, 3)))

        c = row_Cur_3.reshape((p3 + 1) * kw3, -1, 3)
        pool_of_points = np.array(
            [[c[j][i], c[j + 1][i], c[j + 2][i], c[j + 3][i]] for i in range(self.size * kh3) for j in
             range(0, kw3 * p3, p3)])
        Cur3 = np.array(pool.map(self.biz3, pool_of_points))
        return np.append(Cur, Cur3)

    def biz2(self, points):
        a, b, c = points
        r = np.array(
            [a * ((1 - u) * (1 - u)) + (2 * b * u * (1 - u)) + c * (u ** 2) for u in self.t2])
    ...
    def biz3(self, points):
        a, b, c,d = points
        r = np.array(
            [a * ((1 - u) ** 3) + 3 * b * u * ((1 - u) ** 2) + 3 * c * (1 - u) * u ** 2 + d * u ** 3 for u in self.t2])
...
class SimpleGrid(CameraWindow):
    title = "1"
    gl_version = (3, 3)
    resource_dir = (Path(__file__) / '../../1/resources').resolve()
    ...
    def render_ui(self):
        ...
        _, self.shifts = imgui.drag_float3("Translation", *self.shifts,0.1)
        _, self.angles = imgui.drag_float3("Rotation", *self.angles, 0.5,-180, 180,
                                             "%.2f")
        ...
        if changed:
            self.sur.t2 = np.linspace(0, 1, self.sur.size)
