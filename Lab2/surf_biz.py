...
class SimpleGrid(CameraWindow):
    ...
    def key_event(self, key, action, modifiers):
        ...
        if action == keys.ACTION_PRESS:
            ...
            if not self.camera_enabled and self.select_color.shape[0] != 0:
                ...
                self.curu = np.array([])
                self.curu = np.append(self.plot_surf(self.point[:, :, :3][::2]).flatten(), self.curu)
                self.curu = np.append(self.plot_surf(self.point[:, :, :3][1::2]).flatten(), self.curu)
                self.vbo_curu = self.ctx.buffer(self.curu.astype('f4'))
                self.vbo_points.write(self.point.astype("f4"))
            print(self.point[:,:,:3][1::2])
    ...
    def add_surf(self):
        ...
        for x in range(-h, h):
            for y in range(-w, w):
                points = np.append(points, [x, y, np.random.random() * 2])
    ...
    def add_patch(self):
        ...
        for x in range(-h, h):
            for y in range(-5, -1):
                new_points = np.append(new_points, [x, y, np.random.random() * 2])
